...
export class FatigueDetector {
  ...
  update(ear) {
    ...
    // Fatigue score: 0–100. Blends sustained closure (PERCLOS) with an active micro-sleep.
    const microsleep = Math.min(1, this.consecutiveLow / this.drowsyFrames);
    const score = Math.round(
      100 * Math.max(0, Math.min(1, 0.7 * perclos * 3 + 0.3 * microsleep)),
    );

    let status = 'OK';
    if (this.consecutiveLow >= this.drowsyFrames) status = 'DROWSY';
    else if (score > 40) status = 'TIRED';
