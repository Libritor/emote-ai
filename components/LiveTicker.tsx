...
export default function LiveTicker() {
  const { fixtures, mode, loading, error, clockMs } = useFixtures(12000);
  const { asOfIso, asOfMs, exitSimulation, simulationLabel } = useFeedClock();
  const [showSimulator, setShowSimulator] = useState(false);
  const now = asOfMs ?? clockMs ?? 0;
  ...
  if (showSimulator && !asOfIso) return <SimulateFeedPrompt onSimulate={() => setShowSimulator(false)} />;

  if (items.length === 0) {
    if (!loading && !error) return <SimulateFeedPrompt />;

    return (
      <div className="card flex flex-wrap items-center gap-3 px-4 py-2.5 text-sm text-muted">
        <span className="live-dot" />
        <span>{error ? "Feed unavailable. Retrying..." : loading ? "Connecting to feed..." : "No matches on the feed right now."}</span>
        {asOfIso && (
          <button className="chip ml-auto" onClick={exitSimulation}>
            Exit simulation
          </button>
        )}
      </div>
  ...
  return (
    <div className="card flex items-center gap-3 overflow-hidden px-3 py-2" aria-live="polite">
      <span className="flex shrink-0 items-center gap-2 rounded-lg bg-surface-2 px-2.5 py-1 text-xs font-bold uppercase tracking-wide">
        <span className="live-dot" />
        {simulationLabel ? `Simulated as of ${simulationLabel}` : mode === "live" ? "TxODDS Live" : "TxODDS Simulated"}
      </span>
      {asOfIso && (
        <button className="chip shrink-0" onClick={exitSimulation}>
          Exit
        </button>
      )}
      {!asOfIso && (
        <button className="chip shrink-0" onClick={() => setShowSimulator(true)}>
          Simulate date
        </button>
      )}
